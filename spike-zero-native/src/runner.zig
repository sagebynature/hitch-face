    ...
    fn appInfo(self: RunOptions) zero_native.AppInfo {
        return .{
            ...
            .main_window = .{
                ...
                .default_frame = zero_native.geometry.RectF.init(0, 0, face_window_size.width, face_window_size.height),
                .resizable = false,
                .restore_state = false,
            },
        };
...
fn runMacos(app: zero_native.App, options: RunOptions, init: std.process.Init) !void {
    ...
    var app_info = options.appInfo();
    const store = prepareStateStore(init.io, init.environ_map, &app_info, &buffers);
    var mac_platform = try zero_native.platform.macos.MacPlatform.initWithOptions(face_window_size, webEngine(), app_info);
...
fn runLinux(app: zero_native.App, options: RunOptions, init: std.process.Init) !void {
    ...
    var app_info = options.appInfo();
    const store = prepareStateStore(init.io, init.environ_map, &app_info, &buffers);
    var linux_platform = try zero_native.platform.linux.LinuxPlatform.initWithOptions(face_window_size, webEngine(), app_info);
...
fn runWindows(app: zero_native.App, options: RunOptions, init: std.process.Init) !void {
    ...
    var app_info = options.appInfo();
    const store = prepareStateStore(init.io, init.environ_map, &app_info, &buffers);
    var windows_platform = try zero_native.platform.windows.WindowsPlatform.initWithOptions(face_window_size, webEngine(), app_info);
